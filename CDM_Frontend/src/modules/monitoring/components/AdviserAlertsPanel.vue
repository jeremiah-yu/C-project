...
const persistReads = () => localStorage.setItem(READ_KEY, JSON.stringify([...readIds.value]))
const markRead = (id) => {
    ...
    <template v-else-if="payload">
      ...
      <div class="workspace">
        <section class="list-panel">
          ...
          <p v-if="!alerts.length" class="empty">No adviser alerts right now.</p>
        </section>
        ...
        <section v-if="selected" class="detail-panel">
          ...
          <button type="button" class="cta" @click="emit('open-student', selected.student_id)">
            Review in Early Warnings
          </button>
        </section>
      </div>
    </template>
...
<style scoped>
.banner-error, .banner-muted { margin:0 0 16px; padding:12px 14px; border-radius:10px; }
...
.summary-row article, .list-panel, .detail-panel { border:1px solid var(--color-border); border-radius:16px; background:var(--color-surface); padding:16px; box-shadow:var(--shadow-soft); }
.summary-row span { color:var(--color-muted); font-size:.82rem; font-weight:700; }
.summary-row strong { display:block; margin-top:6px; font-family:var(--font-display); font-size:1.6rem; }
.workspace { display:grid; grid-template-columns: minmax(240px,320px) 1fr; gap:16px; }
.list-panel h3, .detail-panel h3, .detail-panel h4 { margin:0 0 12px; font-family:var(--font-display); }
...
.card-btn small, .empty, .message { color: var(--color-muted); }
...
ul { margin:0 0 16px; padding-left:18px; color:var(--color-muted); line-height:1.55; }
...
.page-kicker { margin:0 0 8px; color:var(--color-dartmouth-green); font-size:.82rem; font-weight:700; text-transform:uppercase; }
